...
representation_dict = { x: [[a, b, c] for a in digits for b in digits for c in digits if base**2*a + base**1*b + base**0*c == x][0] for x in range(base**2*(base-1)+base**1*(base-1)+base**0*(base-1)+1) }
...
def all_3_digit_numbers(base, digits): return { base**2*a + base**1*b + base**0*c for a in digits for b in digits for c in digits }
